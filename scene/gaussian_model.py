...
class GaussianModel:
    ...
    def __init__(self, sh_degree: int):
        ...
        self.accum_gs_image_error = torch.empty((0))
        self.accum_gs_edge_error = torch.empty((0))
        self.iterations = 0

        self.burst_k = 500
        self.burst_indices = torch.empty((0,), dtype=torch.long, device='cuda')
        self.burst_errors = torch.empty((0,), dtype=torch.float32, device='cuda')
    ...
    def select_mesh_by_timestep(self):
    ...
    def training_setup(self, training_args):
        ...
        self.accum_gs_image_error = torch.zeros((N), device="cuda")
        self.accum_gs_edge_error = torch.zeros((N), device="cuda")
    ...
    def reset_opacity(self, gs_image_error, error_threshold=5e-4):
        """
        Reset opacity only for points with image error > threshold.

        Args:
            gs_image_error (Tensor): (N,) per-Gaussian image error.
            error_threshold (float): threshold above which opacity is reset.
        """
        current_opacity = self.get_opacity
        mask = gs_image_error > error_threshold

        opacities_new = current_opacity.clone()
        reset_values = torch.ones_like(current_opacity) * 0.01
        opacities_new[mask] = inverse_sigmoid(reset_values[mask])
    ...
    def load_ply(self, path, **kwargs):
        ...
        # * optional fields, binding_names is the face indices that gs binded
        binding_names = [
            p.name
            for p in plydata.elements[0].properties
            if p.name.startswith("binding")
        ]
        if len(binding_names) > 0:
            ...
            self.binding = torch.tensor(
                binding, dtype=torch.int32, device="cuda"
            ).squeeze(-1)
    ...
    def _prune_optimizer(self, mask):
        ...
        for group in self.optimizer.param_groups:
            ...
            if (
                len(group["params"]) != 1
                or group["params"][0].shape[0] != mask.shape[0]
            ):
                continue
    ...
    def prune_points(self, mask):
        ...
        self.accum_gs_image_error = self.accum_gs_image_error[valid_points_mask]
        self.accum_gs_edge_error = self.accum_gs_edge_error[valid_points_mask]
    ...
    def densify_and_split(
        ...
        selected_pts_mask = torch.logical_and(selected_pts_mask, size_mask)
        ...
        noise_strength_dc = 0.05
        noise_strength_rest = 0.05
        new_features_dc = self._features_dc[selected_pts_mask].repeat(N, 1, 1)
        new_features_dc += torch.randn_like(new_features_dc) * noise_strength_dc
        new_features_rest = self._features_rest[selected_pts_mask].repeat(N, 1, 1)
        new_features_rest += torch.randn_like(new_features_rest) * noise_strength_rest

        new_opacity = self.inverse_opacity_activation(1 - torch.sqrt(1 - self.opacity_activation(self._opacity[selected_pts_mask]))).repeat(N, 1)
    ...
    def densify_and_clone_error(self, gs_image_error, image_error_threshold=1e-3):
        #======== Select points by threshold ========
        high_err_idx = torch.nonzero(gs_image_error > image_error_threshold, as_tuple=False).squeeze(1)
        if high_err_idx.shape[0] > 2500:
            _, high_err_idx = torch.topk(gs_image_error, 2500, largest=True)

        #======== Merge with burst indices ========
        burst_idx = getattr(self, "burst_indices", torch.empty((0,), dtype=torch.long, device=gs_image_error.device))
        all_idx = torch.cat([high_err_idx, burst_idx], dim=0)
        all_idx = torch.unique(all_idx)
        selected_pts_mask = torch.zeros_like(gs_image_error, dtype=torch.bool, device=gs_image_error.device)
        selected_pts_mask.view(-1)[all_idx] = True

        #======== Gather attributes for the selected points ========
        features_dc   = self._features_dc[selected_pts_mask]
        features_rest = self._features_rest[selected_pts_mask]
        scaling_sel   = self._scaling[selected_pts_mask]
        rotation_sel  = self._rotation[selected_pts_mask]

        #======== Apply small random perturbations ========
        pos_noise = torch.normal(mean=0.0, std=self.scaling_activation(scaling_sel) * 0.05)
        new_xyz = self._xyz[selected_pts_mask] + pos_noise

        feat_noise_dc   = torch.randn_like(features_dc) * 0.05
        feat_noise_rest = torch.randn_like(features_rest) * 0.05
        new_features_dc   = features_dc   + feat_noise_dc
        new_features_rest = features_rest + feat_noise_rest
        ...
        if self.binding is not None:
            binding_sel = self.binding[selected_pts_mask]
            self.binding = torch.cat((self.binding, binding_sel), dim=0)
            self.binding_counter.scatter_add_(
                0,
                binding_sel,
                torch.ones_like(binding_sel, dtype=torch.int32, device=binding_sel.device),
            )
    ...
    def densify_and_prune(
        ...
        gs_image_error = self.accum_gs_image_error / self.iterations

        #======== error-based adc =========
        self.densify_and_clone_error(gs_image_error)
    ...
    def add_densification_stats(
        ...
        self.accum_gs_image_error += gs_image_error
        self.iterations += 1

        flat_errors = gs_image_error.flatten()
        curr_vals, curr_idx = torch.topk(flat_errors, self.burst_k, largest=True, sorted=False)

        all_idx = torch.cat((self.burst_indices, curr_idx), dim=0)
        all_err = torch.cat((self.burst_errors, curr_vals), dim=0)

        unique_idx, inverse_map = torch.unique(all_idx, return_inverse=True)
        max_err = torch.zeros_like(unique_idx, dtype=all_err.dtype)
        max_err, _ = torch_scatter.scatter_max(all_err, inverse_map)

        K = min(self.burst_k, unique_idx.shape[0])
        top_vals, top_positions = torch.topk(max_err, K, largest=True, sorted=False)
        self.burst_indices = unique_idx[top_positions]
        self.burst_errors  = top_vals